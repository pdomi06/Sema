...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (interaction.user.id === "715130502335561749") {
        // no one
      ...
      ch.send({
        content: `**${interaction.user.tag}** executed \`${interaction.commandName}\` in **${interaction.guild.name}** (interaction)`,
      });
    }
    ...
    if (interaction.isButton()) {
      ...
      ch.send({
        content: `**${interaction.user.tag}** executed \`${button}\` in **${interaction.guild.name}** (button)`,
      });
    }
  },
};

...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const profile =
      locales[`${interaction.locale}_guildjs_profile`] ?? " guild infos";
    const id = locales[`${interaction.locale}_guildjs_id`] ?? "Id";
    const owner = locales[`${interaction.locale}_guildjs_owner`] ?? "Owner";
    const memberCount =
      locales[`${interaction.locale}_guildjs_memberCount`] ?? "Member count";
    const afkTimeout =
      locales[`${interaction.locale}_guildjs_afkTimeout`] ?? "AFK timeout";
    const afkChannel =
      locales[`${interaction.locale}_guildjs_afkChannel`] ?? "AFK channel";
    const premiumCount =
      locales[`${interaction.locale}_guildjs_premiumCount`] ??
      "Server boost count";
    const premiumTier =
      locales[`${interaction.locale}_guildjs_premiumTier`] ??
      "Server boost tier";
    const createdAt =
      locales[`${interaction.locale}_guildjs_createdAt`] ?? "Server created at";
    ...
    const guildEmbed = new EmbedBuilder()
      ...
      .addFields(
        ...
        {
          name: `💤 | ${afkTimeout}: `,
          value: `${interaction.guild.afkTimeout / 60}`,
          inline: true,
        },
        {
          name: `🛏️ | ${afkChannel}: `,
          value: `<#${interaction.guild.afkChannelId}>`,
          inline: true,
        },
        {
          name: `💎 | ${premiumCount}: `,
      ...
      .setFooter({
        text: " ",
        iconURL: `https://cdn.discordapp.com/avatars/338739294753783818/${interaction.user.avatar}.png?size=1024`,
      });

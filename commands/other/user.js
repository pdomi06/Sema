...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption((option) =>
      option
        ...
        .setDescription("Choose user to get informations.")
        .setDescriptionLocalizations({
  ...
  async execute(interaction) {
    const profile =
      locales[`${interaction.locale}_userjs_profile`] ?? "'s profile";
    const id = locales[`${interaction.locale}_userjs_id`] ?? "Id";
    const nickname =
      locales[`${interaction.locale}_userjs_nickname`] ?? "Nickname";
    const Tag = locales[`${interaction.locale}_userjs_tag`] ?? "Tag";
    const bot = locales[`${interaction.locale}_userjs_bot`] ?? "Bot";
    const joinedAt =
      locales[`${interaction.locale}_userjs_joinedAt`] ??
      "Joined to the server";
    const createdAt =
      locales[`${interaction.locale}_userjs_createdAt`] ?? "Account created at";
    ...
    const userEmbed = new EmbedBuilder()
      ...
      .addFields(
        ...
        {
          name: `📅 | ${joinedAt}: `,
          value: new Date(mentioned.member.joinedAt).toLocaleDateString(),
        },
        {
          name: `📅 | ${createdAt}: `,
          value: new Date(mentioned.user.createdAt).toLocaleDateString(),
        },
      )
      .setTimestamp()
    ...
    await interaction.reply({
      embeds: [userEmbed],
      ephemeral: ephemeral_option.value,
    });
  },
};

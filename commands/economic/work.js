...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription(
      "Earn money by working. DISCLAIMER: **IF YOU WORK IT'LL DISABLE SOME FUNCTIONS!**",
    )
  ...
  async execute(interaction) {
    ...
    if (setup === true) {
      ...
      if (workFinish < Date.now() && access === true) {
        ...
        const prof =
          locales[`${interaction.locale}_workjs_profile`] ?? "'s chief";
        const paymentBefore =
          locales[`${interaction.locale}_workjs_paymentBefore`] ??
          "Chief: -- I already calculated your payment.";
        const paymentOngoing =
          locales[`${interaction.locale}_workjs_paymentOngoing`] ??
          "Me: -- How much will I get chief?";
        const paymentAfter =
          locales[`${interaction.locale}_workjs_paymentAfter`] ??
          "Chief: -- Estimatedly";
        const workLenght =
          locales[`${interaction.locale}_workjs_lenght`] ??
          `You going to work for \`${
            h * 60 + m
          }\` minutes, which means you get a \`${multiplier}x\` multiplier.`;
        ...
        await interaction.reply(
          "You didn't finish working yet. Use /claim or /end_work to finish.",
        );
      }
    } else {
      await interaction.reply(

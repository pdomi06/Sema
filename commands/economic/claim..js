		...
		if (setup == true) { 
			...
			if (rights == true) { 
				...
				if (ready <= Date.now()) { 
					...
					const payoutBefore = locales[`${interaction.locale}_claimjs_payoutBefore`] ?? "Chief: -- Good job my little worker. Now you deserv your paycheck." 
					const payoutOngoing = locales[`${interaction.locale}_claimjs_payoutOngoing`] ?? "Me: -- Thanks chief! Can I ask you how much do I get again?" 
					const payoutAfter = locales[`${interaction.locale}_claimjs_payoutAfter`] ?? "Chiefe: -- Yeah sure, you get:" 
					...
					const readyFormated = await db.get(`${interaction.user.id}.work_finish_date`) 
					await interaction.reply(`You can collect your goods at \`${readyFormated}\``) 
				} 

...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription(
      "Provides informations about a stock on eotvos.projekt9.hu.",
    )
  ...
  async execute(interaction) {
    ...
    const options = {
      ...
      headers: {
        ...
        cookie:
          "token=HPp%40lzat1PIf7n15Ct-MYPna%401Xm9bq%40f; username=patkosdominik; osztaly=2021I; id=223",
    ...
    const req = https.request(options, (res) => {
      ...
      res.on("end", async () => {
        ...
        const prof =
          locales[`${interaction.locale}_stockjs_profile`] ?? "Eötvös stock";
        const stockEmbed = new EmbedBuilder()
          .setColor(0xffffff)
          .setTitle(prof)
          .setThumbnail(
            "https://www.eotvosszki.hu/_next/image?url=https%3A%2F%2Fbg-eotvos.cms.intezmeny.edir.hu%2Fuploads%2Fthumbnail_elg_logo_8fc4582687.PNG&w=128&q=90",
          );
        ...
        for (let i = 0; i < listN.length; i++) {
          ...
          stockEmbed.addFields(
            {
              name: `${listN[i]} részvény: `,
              value: `${listP[i]} - ${sellP.toFixed(2)}`,
            },
            {
              name: `10db ${listN[i]} részvény: `,
              value: `${listP[i] * 10} - ${sellP.toFixed(2) * 10}`,
            },
          );
        }

        stockEmbed.setTimestamp().setFooter({ text: " " });

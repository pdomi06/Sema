...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        ...
        .setRequired(true)
        .addChoices({ name: "Be", value: "be" }, { name: "Ki", value: "ki" }),
    )
    .addStringOption((option) =>
      option
        .setName("item")
    ...
    .addIntegerOption((option) =>
      option.setName("mennyiseg").setDescription("mennyi").setRequired(true),
    ),
  ...
  async execute(interaction) {
    ...
    if (type === "be") {
      ...
      const existingDocument_K = await collection_K.findOne({
        Name: interaction.user.username,
      });

      if (existingDocument_T) {
        // If document exists, update the "Value" field
        await collection_T.updateOne(
          { Item: targy },
          { $inc: { Value: mennyiseg } },
        );
      ...
      if (existingDocument_K) {
        ...
        await collection_K.updateOne(
          { Name: interaction.user.username },
          { $inc: { Value: mennyiseg, Money: mennyiseg * piros_ken } },
        );
        console.log(`Updated existing document for ${targy} For piros-kén.`);
      } else {
        // If document doesn't exist, create a new one
        await collection_K.insertOne({
          Name: interaction.user.username,
          Value: mennyiseg,
          Money: mennyiseg * piros_ken,
        });
      ...
      if (existingDocument_T) {
        ...
        if (existingDocument_T.Value < mennyiseg) {
          await interaction.reply(
            "Többet szeretnél ki venni mint amennyi van benne!",
          );
        } else {
          await collection_T.updateOne(
            { Item: targy },
          ...
          console.log(
            `Subtracted ${mennyiseg} from existing document for ${targy}.`,
          );
